...
_DEFAULT_DATA = {
    "setup_channels": {"repeat": {}, "nickname": {}, "modlog": {}, "announce": {}},
    "nickname_requests": {},
    "next_request_id": 1,
}
...
def load_data() -> dict:
    if DATA_FILE.exists():
        try:
            ...
            for kind in ("repeat", "nickname", "modlog", "announce"):
...
# /setup  (admin only)
# ---------------------------------------------------------------------------
@bot.hybrid_command(name="setup", description="[Admin] Configure a single-purpose channel.")
@discord.app_commands.describe(
...
async def setup_cmd(
    ...
    kind: Literal["repeat", "nickname", "modlog", "announce"],
...
async def setup_cmd_error(ctx: commands.Context, error):
    if isinstance(error, commands.MissingPermissions):
        await _reply(ctx, "Only server administrators can use `setup`.")
    elif isinstance(error, commands.MissingRequiredArgument):
        await _reply(ctx, "Usage: `/setup kind:<repeat|nickname|modlog|announce> channel:#channel`")
    else:
        raise error
...
# ---------------------------------------------------------------------------
@bot.hybrid_command(name="dev", description="Developer credit.")
@discord.app_commands.default_permissions(administrator=True)
async def dev(ctx: commands.Context):
    if not DEV_USER_ID:
        await _reply(ctx, "DEV_USER_ID isn't configured yet.")
        return
    await ctx.send(f"THIS BOT IS DEVELOPED BY <@{DEV_USER_ID}>")
